...
#"cantidad_de_dias_en_el_anio" no esta definida, dara error
...
def temporada_rango_especial():
    return 2022
    return 2021  # ¿Se llega a ejecutar esta línea? No, pq solo puede haber un return (excepto si hay uno o más if(s))
...
# No se ejecutara, ya que solo puede haber un return
...
#Imprimira 24 y la palabra "None" ya que no tiene parametros
...
#Dara error, especificamente " unsupported operand type(s) for +: 'NoneType' and 'NoneType'", ya que "suma_puntos(10, 5) + suma_puntos(4, 3)" deberia estar en un return
...
#En efecto, se combinaran en orden inverso, especificamente, imprimira "#Diversión#Verano"
...
#No, solo puede haber un return por cada if o else
...
#No, ya que solo puede haber un return por cada if o else (esto ya lo dije xd)
...
#No hay condicionales para ejecutar ese return(ojo, si se puede ejecutar un return sin un condicional, dos, no)
...
#Imprimira "150 150 350 150"

    #Hasta aqui nomas llegue profe sorry
